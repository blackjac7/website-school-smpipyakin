  ...
  const parseFile = async (file: File) => {
    try {
      ...
      if (missingColumns.length > 0) {
        setValidationErrors([
          `Kolom wajib hilang: ${missingColumns.join(", ")}`,
        ]);
        setPreviewData([]);
      } else {
        setValidationErrors([]);
          ...
          <div className="bg-blue-50 p-4 rounded-lg flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700 mt-1 mb-2">
                Gunakan template Excel standar untuk menghindari kesalahan
                import. Kolom Tanggal Lahir (YYYY-MM-DD) wajib diisi untuk
                generate password.
              </p>
              <a
