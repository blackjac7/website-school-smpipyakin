...
import { upsertAdzanSchedule, deleteAdzanSchedule, updateAdzanStatus } from "@/actions/worship";
...
export default function AdzanTab({ schedules }: AdzanTabProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    prayerTime: "ZUHUR" as PrayerTime
  });
  ...
  return (
    <div className="space-y-6">
       <div className="flex justify-between items-center">
        <div>
            <h3 className="text-lg font-semibold text-gray-700">Jadwal Muadzin</h3>
            <p className="text-sm text-gray-500">Petugas Adzan Zuhur, Ashar, dan Jumat</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedDates.length === 0 ? (
            ...
            sortedDates.map(dateKey => {
                ...
                return (
                    <div key={dateKey} className={`bg-white rounded-xl shadow-sm border ${isToday ? 'border-blue-500 ring-1 ring-blue-500' : 'border-gray-200'} overflow-hidden`}>
                        <div className={`px-4 py-3 border-b ${isToday ? 'bg-blue-50 text-blue-800' : 'bg-gray-50 text-gray-700'} font-medium flex justify-between items-center`}>
                            <span>{format(dateObj, "EEEE, d MMM", { locale: idLocale })}</span>
                            {isToday && <span className="text-xs bg-blue-200 text-blue-800 px-2 py-0.5 rounded-full">Hari Ini</span>}
                        </div>
                        <div className="divide-y divide-gray-100">
                            {daySchedules.map(sch => (
                                <div key={sch.id} className="p-4 flex items-start gap-3 group hover:bg-gray-50 transition-colors">
                                    <button
                                    ...
                                    >
                                        {sch.status === 'COMPLETED' ? <CheckCircle2 size={20} /> : <Circle size={20} />}
                                    </button>
                                    <div className="flex-1">
                                        <div className="flex justify-between">
                                            <span className="text-xs font-bold uppercase tracking-wider text-gray-500">{sch.prayerTime}</span>
                                        </div>
                                        <div className="font-medium text-gray-900">{sch.siswa.name}</div>
                                        <div className="text-xs text-gray-500">{sch.siswa.class}</div>
                                    </div>
                                    <button
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Waktu Sholat</label>
                <select
                    className="w-full p-2 border border-gray-300 rounded-md"
                    value={formData.prayerTime}
                    onChange={(e) => setFormData({...formData, prayerTime: e.target.value as PrayerTime})}
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Petugas (Siswa)</label>
                <StudentSelector
