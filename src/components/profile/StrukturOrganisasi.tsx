...
export default function StrukturOrganisasi({
  ...
  const guruCount = teachers.filter(
    (t) => t.category === "Guru Mata Pelajaran"
  ).length;
  const staffCount = teachers.filter((t) => t.category === "Staff").length || 5;

  // Default avatar fallback
  const defaultAvatar =
    "https://ui-avatars.com/api/?name=User&size=400&background=1E40AF&color=fff&bold=true";
          ...
          <div className="md:hidden flex flex-col items-center space-y-8">
            {wakaList.map((waka) => (
              <div key={waka.id} className="flex flex-col items-center">
                <OrgCard
                  image={waka.photo || defaultAvatar}
                  name={waka.name}
                  position={waka.position}
                />
                <VerticalLine />
              </div>
            ))}
